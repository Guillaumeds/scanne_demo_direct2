#!/usr/bin/env python3
"""
Extract all images from M-series sugarcane variety PDFs into organized folders

This script:
1. Finds all M-series PDFs in the sugarcane_varieties_leaflets folder
2. Creates a subfolder for each variety
3. Extracts all images and saves them with descriptive names
4. Provides a summary of extraction results
"""
...
def extract_images_from_pdf(pdf_path, output_dir, variety_name):
    ...
    Args:
        ...
        variety_name (str): Name of the variety for naming
...
def main():
    ...
    # Find all M-series PDFs (avoid duplicates)
    m_series_pdfs = []
    processed_varieties = set()

    # Process M *.pdf files (with space)
    for pdf_file in pdf_dir.glob("M *.pdf"):
        variety_name = pdf_file.stem.replace("M ", "M")  # M 683-99 -> M683-99
        if variety_name not in processed_varieties:
            m_series_pdfs.append((pdf_file, variety_name))
            processed_varieties.add(variety_name)

    # Also check for m-series (lowercase)
    for pdf_file in pdf_dir.glob("m*.pdf"):
        variety_name = pdf_file.stem.upper()  # m2593 -> M2593
        if variety_name not in processed_varieties:
            m_series_pdfs.append((pdf_file, variety_name))
            processed_varieties.add(variety_name)
    ...
    print(f"Found {len(m_series_pdfs)} M-series PDFs to process:")
    for pdf_file, variety_name in m_series_pdfs:
