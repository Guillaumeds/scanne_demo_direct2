...
/**
 * Mock soil data for development/testing
 * @param coordinates [longitude, latitude]
 * @returns Promise<SoilAnalysis>
 */
export async function getMockSoilAnalysis(coordinates: [number, number]): Promise<SoilAnalysis> {
  // Simulate API delay
  ...
  // Generate soil properties based on location
  // Different latitudes and longitudes have different soil characteristics
  const clayBase = 15 + coordSeed * 30 // 15-45% clay
  const sandBase = 20 + (1 - coordSeed) * 40 // 20-60% sand
  const siltBase = 100 - clayBase - sandBase // Remaining percentage

  const mockData: SoilGridsData = {
    phh2o: Math.max(4.5, Math.min(8.5, 5.5 + coordSeed * 2.5)), // pH 5.5-8.0
    sand: Math.max(10, Math.min(70, sandBase + (coordSeed - 0.5) * 20)),
    silt: Math.max(10, Math.min(60, siltBase + (coordSeed - 0.5) * 15)),
    clay: Math.max(5, Math.min(50, clayBase + (coordSeed - 0.5) * 10)),
    soc: Math.max(5, Math.min(40, 10 + coordSeed * 25)), // Organic carbon 5-35 g/kg
    nitrogen: Math.max(0.5, Math.min(4.0, 1.0 + coordSeed * 2.5)), // Nitrogen 0.5-3.5 g/kg
    cec: Math.max(8, Math.min(35, 12 + coordSeed * 20)), // CEC 8-32 cmol/kg
    bdod: Math.max(1.0, Math.min(1.8, 1.2 + coordSeed * 0.4)), // Bulk density 1.0-1.6 kg/dm³
    cfvo: Math.max(0, Math.min(25, coordSeed * 20)) // Coarse fragments 0-20% volume
  }

  const soilType = determineSoilType(mockData.sand, mockData.silt, mockData.clay)
  const fertility = assessFertility(mockData)
  const recommendations = generateRecommendations(mockData, soilType, fertility)

  return {
    coordinates,
    soilGridsData: mockData,
    soilType,
    fertility,
    recommendations,
    lastUpdated: new Date().toISOString()
